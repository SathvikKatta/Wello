...
export default function FoodDetailScreen() {
 ...
 const geminiOutput = params.analysis as string;
 const titleParam = params.title as string;
 const rating = params.rating as string;
 const extractedRating = rating?.match(/(\d+\/\d+)/)?.[1] || "";
 ...
 const pros = geminiOutput?.match(/\*\*Pros(?: of.*?)?\*\*\s*([\s\S]*?)\s*\*\*Cons(?: of.*?)?\*\*/)?.[1]
   ...
   ?.trim()
   ?.split(/\n(?=\d+\.\s)/)  // split like before
   ?.filter(line => !/summary|overview/i.test(line)) // ✨ filter out unwanted lines
   || [];
       ...
       <View style={styles.headerCard}>
         ...
         <View style={{ flex: 1, marginLeft: 12 }}>
           ...
           <View style={styles.ratingRow}>
             ...
             <Text style={styles.ratingText}> Rating: {extractedRating}</Text>
           </View>
         </View>
